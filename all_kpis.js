...
function reportValue(id,q){const k=MOU_DATA.kpis[id];if(!k?.isLeaf)return null;const report=publishedQuarterReport(id,q);if(report?.actual!==undefined)return report.actual;if(q==='q1'||q==='q2')return MOU_DATA.quarterly[id]?.[q]?.actual??null;return getPublicQuarterInput(id,q);}
...
function reportDetailHtml(id){const k=getEffectiveKpi(id),q=dtlQuarter,leaf=k.isLeaf,s=leaf?scoreAt(id,q):scoreParentAt(id,q),fc=forecastHomeScore(id),report=publishedQuarterReport(id,q),children=ovpChildren(id),leaves=reportLeaves(id),filled=leaves.filter(c=>getPublicQuarterInput(c.id,q)!==null).length;
 const scoreColorNow=s.level!==null?lvColor(s.level):LV_COLORS[null];
 const scoreColorFc=(fc&&fc.level!==null&&fc.level!==undefined)?lvColor(fc.level):LV_COLORS[null];
 const activities=MOU_DATA.activities[id]?.steps||[];
 const rows=QUARTERS.map(qq=>{const rs=leaf?scoreAt(id,qq):scoreParentAt(id,qq);const rowCls=qq===q?'current-q':'';return `<tr class="${rowCls}"><td>${Q_LABEL[qq]}</td><td>${leaf?reportValueText(id,qq):'<span class="ovp-cell-na" title="ไม่มีผลเดี่ยว — เป็นค่ารวมถ่วงน้ำหนักจากตัวชี้วัดย่อย">—</span>'}</td><td>${ovpBadge(rs.level)}</td><td>${ovpBadge(k.targetScore)}</td></tr>`}).join('');
 ...
 return `<div class="dtl-header-card"><div class="dtl-page-title">รายละเอียดตัวชี้วัด</div><div class="dtl-breadcrumb">${entryEsc(k.groupLabel||'')} · ตัวชี้วัด ${id}</div><div class="dtl-h1"><span class="dtl-id-badge">${id}</span>${entryEsc(k.label)}</div><div class="dtl-meta-grid"><div class="dtl-meta-item">น้ำหนัก<b>${k.weight}%</b></div><div class="dtl-meta-item">หน่วย<b>${entryEsc(k.unit||'—')}</b></div><div class="dtl-meta-item">เป้าหมาย<b>${entryEsc(String(k.target??'—'))}</b></div></div><div class="dtl-quarter-tabs">${QUARTERS.map(qq=>`<button class="dtl-qbtn ${q===qq?'active':''}" onclick="dtlSetQuarter('${qq}')">${Q_LABEL[qq]} (${PERIOD_LABEL[qq]})</button>`).join('')}</div><div class="dtl-source-note">มีผล ${filled}/${leaves.length} ตัวชี้วัดย่อย</div><button class="cta-btn" onclick="reportToEntry('${id}')">ไปกรอกผล Q3 →</button></div>
 <div class="dtl-cards"><div class="dtl-card c-actual"${leaf?'':` style="border-left-color:${scoreColorNow}"`}><div class="dtl-card-label">${leaf?'ผลการดำเนินงาน':'คะแนนรวมตัวชี้วัดย่อย'} ${Q_LABEL[q]}</div><div class="${leaf&&typeof reportValue(id,q)==='string'?'dtl-support-val':'dtl-card-val'}"${leaf?'':` style="color:${scoreColorNow}"`}>${leaf?reportValueText(id,q):s.level===null?'—':s.level.toFixed(4)}</div><div class="dtl-card-sub">${filled<leaves.length?'ข้อมูลยังไม่ครบ':''}</div></div><div class="dtl-card c-score" style="border-left-color:${scoreColorNow}"><div class="dtl-card-label">คะแนน ${Q_LABEL[q]}</div><div class="dtl-card-val" style="color:${scoreColorNow}">${s.level===null?'—':s.level.toFixed(4)}</div></div><div class="dtl-card c-fcresult"><div class="dtl-card-label">คาดการณ์สิ้นปี</div><div class="dtl-support-val">${leaf?entryEsc(String(MOU_DATA.forecast[id]?.result??'—')):'<span class="ovp-cell-na" title="ไม่มีผลเดี่ยว — เป็นค่ารวมถ่วงน้ำหนักจากตัวชี้วัดย่อย">—</span>'}</div></div><div class="dtl-card c-fcscore" style="border-left-color:${scoreColorFc};background:color-mix(in srgb, ${scoreColorFc} 14%, white)"><div class="dtl-card-label">คะแนนคาดการณ์</div><div class="dtl-card-val" style="color:${scoreColorFc}">${fc.level===null?'—':fc.level.toFixed(4)}</div></div></div>
 ...
 <div class="dtl-section-card"><div class="card-title">สรุปผลและประเด็นติดตาม</div>${reportSummaryHtml(id,q)||`<div class="dtl-support-val empty">${entryEsc(MOU_DATA.quarterly[id]?.[q]?.note||'ยังไม่มีข้อความที่ยืนยันในไตรมาสนี้')}</div>`}${breakdown?`<div class="dtl-meta-grid">${breakdown}</div>`:''}</div>
